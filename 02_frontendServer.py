...
# 轮询等待任务完成并获取结果
def poll_task_result(user_id: str, session_id: str, task_id: str, max_wait: int = 120):
    """
    轮询等待Celery任务完成，并返回最终结果
    ...
    Returns:
        任务完成后的状态响应，或超时时返回None
    """
    poll_interval = 2  # 每2秒轮询一次
    elapsed = 0

    with Progress() as progress:
        poll_task = progress.add_task("[cyan]智能体处理中...", total=None)

        while elapsed < max_wait:
            try:
                status_response = get_agent_status(user_id, session_id, task_id)

                if status_response["status"] in ("completed", "interrupted", "error", "failed", "not_found"):
                    progress.update(poll_task, completed=100)
                    return status_response
                elif status_response["status"] == "running":
                    progress.update(poll_task, description="[cyan]智能体正在思考中...")
                elif status_response["status"] == "pending":
                    progress.update(poll_task, description="[cyan]任务排队等待中...")
                else:
                    progress.update(poll_task, description=f"[cyan]当前状态: {status_response['status']}...")

                time.sleep(poll_interval)
                elapsed += poll_interval

            except Exception as e:
                console.print(f"[warning]轮询任务状态时出错: {str(e)}，继续等待...[/warning]")
                time.sleep(poll_interval)
                elapsed += poll_interval

        progress.update(poll_task, completed=100)
        console.print("[warning]等待任务完成超时[/warning]")
        return None
...
def process_agent_response(response, user_id):
    ...
    try:
        ...
        # 轮询等待任务完成
        final_status = poll_task_result(user_id, session_id, task_id)

        if final_status is None:
            console.print(Panel(
                f"任务已提交，但等待超时。\n任务ID: {task_id}\n之后可通过 'history' 命令查询结果。",
                title="[warning]任务仍在处理中[/warning]",
                border_style="yellow"
            ))
            return task_id
        ...
        if status == "completed":
            last_response = final_status.get("last_response", {})
            if last_response and last_response.get("result"):
                result = last_response["result"]
                if "messages" in result:
                    final_message = result["messages"][-1]
                    console.print(Panel(
            ...
            console.print(f"[info]任务 {task_id} 已完成[/info]")

        elif status == "interrupted":
            console.print(Panel(
                "智能体执行被中断，需要您的决策才能继续。",
                title="[warning]任务中断[/warning]",
                border_style="yellow"
            ))
            # 尝试自动恢复中断
            last_response = final_status.get("last_response", {})
            if last_response and last_response.get("interrupt_data"):
                result = process_agent_response_resume(last_response, user_id)

        elif status == "error" or status == "failed":
            console.print(Panel(
                f"任务执行失败: {final_status.get('message', '未知错误')}",
                title="[error]任务错误[/error]",
                border_style="red"
            ))

        elif status == "not_found":
            console.print(Panel(
                f"任务 {task_id} 未找到，可能已被清理或过期。",
                title="[warning]任务未找到[/warning]",
                border_style="yellow"
            ))
